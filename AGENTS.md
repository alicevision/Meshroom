...
│   │   │   └── Docking/        # Dockable panels: DockManager, DockArea, DockGroup (tabs), DockPanel, workspaces
...
* **Dockable panels.** The main panels (Image Gallery, Image/Text/3D Viewers, Graph Editor, Task Manager, Script Editor, Node Editor) are `DockPanel`s declared once in `Application.qml`/`WorkspaceView.qml` and never destroyed: the QML `DockManager` (`meshroom/ui/qml/Docking/`) reparents them into groups of tabs following a layout tree of splits and tabs. The tree lives on the Python side: `meshroom/ui/dockLayout.py` (Qt-free, unit tested: moves, open/closed panels, sizes, validation of stored layouts) exposed and persisted in the settings by `meshroom/ui/dockLayoutManager.py` (`_dockLayout` in QML), which also stores named workspaces (View > Workspaces). A panel must never leave its window, even transiently (the Qt3D `Scene3D` of the 3D Viewer crashes): panels are parked in their `DockArea` before any rebuild.
