...
from meshroom.core.attribute import ListAttribute, Attribute, CustomAttributes
...
class AddEdgeCommand(GraphCommand):
    ...
    def redoImpl(self) -> bool:
        try:
            srcAttribute = self.graph.attribute(self.srcAttr)
            dstAttribute = self.graph.attribute(self.dstAttr)
...
class ConnectDynamicAttributeCommand(GraphCommand):
    """
    ...
    def redoImpl(self) -> bool:
        ...
        if not isinstance(dynamicAttr.root, CustomAttributes):
            return
